...
- Store the plugin configuration as JSON (`config/smartmeter.json`) instead of an INI file. Global settings live in `MAIN` and `VZLOGGER`, while every reader lives below a `METERS` object.
